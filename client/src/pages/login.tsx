...
// Se quiser restringir, use ["ecobrasil.bio.br"].
// Se quiser permitir múltiplos domínios corporativos, liste todos.
const ALLOWED_DOMAINS: string[] = ["ecobrasil.bio.br"];
...
const validatePassword = (password: string) => {
  ...
  if (score < 3) return "Use pelo menos 3 destes: letra minúscula, maiúscula, número, símbolo";
...
export default function Register() {
  ...
  // Segurança: auto-registro não deveria permitir cargo alto.
  // Melhor padrão: backend define "colaborador" e admin promove depois.
  const [cargo, setCargo] = useState<string>("colaborador");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const domain = emailDomain(finalEmail);
    ...
    // Mesmo que o usuário tente burlar, trate como padrão seguro.
    // Ideal: backend ignora cargo vindo do cliente.
    const safeCargo = cargo || "colaborador";
    ...
    try {
      ...
      toast({ title: "Conta criada com sucesso!", description: "Bem-vindo ao sistema!" });
      setLocation("/");
    } catch (err: any) {
      // Se seu backend devolver status, dá para mapear melhor:
      // 409: email já existe, 400: validação
        ...
        <Card className="w-full shadow-2xl backdrop-blur-lg bg-white/20 dark:bg-white/10 border border-white/30 dark:border-white/20">
          <CardContent className="pt-8 pb-8 px-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                {email && (
                  <p className="text-xs text-white/70 mt-2">
                    Será registrado como: <span className="font-medium text-white/90">{normalizedEmail}</span>
                  </p>
                )}
              </div>
              ...
              <div>
                <Label htmlFor="confirmPassword" className="block text-sm font-medium text-white mb-2 drop-shadow">
                  Confirmar Senha
                </Label>
                <div className="relative">
                  <Input
              ...
              <div>
                <Label htmlFor="cargo" className="block text-sm font-medium text-white mb-2 drop-shadow">
                  <UserCircle className="h-4 w-4 inline mr-2" />
                  Cargo
                </Label>

                {/* Sugestão de segurança: auto-registro apenas como colaborador */}
                <Select value={cargo} onValueChange={setCargo} disabled={isLoading}>
                  ...
                  <SelectContent>
                    ...
                    {/* Se quiser liberar via convite/admin, ative depois.
                        No auto-registro, isso não é recomendado. */}
