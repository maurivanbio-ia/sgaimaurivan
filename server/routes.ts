...
// Import domain route modules (refatoração progressiva de God File)
import { registerAuthRoutes } from "./routes/auth.routes";
import { registerEmpreendimentosRoutes } from "./routes/empreendimentos.routes";
import { registerDemandasRoutes } from "./routes/demandas.routes";
import { registerLicencasRoutes } from "./routes/licencas.routes";
import { registerFinanceiroRoutes } from "./routes/financeiro.routes";
import { registerEquipamentosRoutes } from "./routes/equipamentos.routes";
import { registerFrotaRoutes } from "./routes/frota.routes";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ── Auth routes → extraído para server/routes/auth.routes.ts ──────────────
  registerAuthRoutes(app, { storage, requireAuth });

  // ── Empreendimentos routes → extraído para server/routes/empreendimentos.routes.ts ─
  registerEmpreendimentosRoutes(app, { storage, requireAuth });



  // ── Licenças routes → extraído para server/routes/licencas.routes.ts ────────
  registerLicencasRoutes(app, { storage, requireAuth });

  // ── Demandas routes → extraído para server/routes/demandas.routes.ts ────────
  registerDemandasRoutes(app, { storage, requireAuth });
  ...
  // ── Financeiro routes → extraído para server/routes/financeiro.routes.ts ────
  registerFinanceiroRoutes(app, { storage, requireAuth, requireSensitiveUnlock });

  // ── Equipamentos routes → extraído para server/routes/equipamentos.routes.ts ─
  registerEquipamentosRoutes(app, { storage, requireAuth });

  // ── Frota routes → extraído para server/routes/frota.routes.ts ─────────────
  registerFrotaRoutes(app, { storage, requireAuth });
