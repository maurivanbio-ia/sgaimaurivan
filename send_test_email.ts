...
async function sendTestEmail() {
  const targetEmail = 'maurivan@ecobrasil.bio.br';
  const ano = new Date().getFullYear();
  
  console.log(`Buscando usuário ${targetEmail}...`);
  
  const [usuario] = await db.select().from(users).where(eq(users.email, targetEmail)).limit(1);
...
async function sendEmail(email: string, stats: any, ano: number) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM || 'noreply@ecobrasil.bio.br',
    to: email,
...
sendTestEmail().then(() => { console.log('Processo concluído'); process.exit(0); }).catch(e => { console.error('Erro:', e); process.exit(1); });
