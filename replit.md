...
The frontend uses React, TypeScript, Vite, Wouter for routing, TanStack Query for state management, styled with Shadcn/UI and Tailwind CSS. Forms are managed by React Hook Form with Zod validation. The backend uses Express.js with TypeScript, PostgreSQL via Drizzle ORM, and session-based authentication with `express-session`, `connect-pg-simple` (PostgreSQL session store — persists across restarts), and `bcrypt`. File uploads are handled by Multer with `memoryStorage` (never local disk); all uploaded files are persisted in **Object Storage** (`PRIVATE_OBJECT_DIR/arquivos/timestamp_name`) via `arquivoController.ts`. The `caminho` column in the `arquivos` table stores a portable reference prefixed with `object:` (e.g. `object:arquivos/1234_file.pdf`). Legacy files with local paths return a descriptive 404 message asking the user to re-upload. Job scheduling is handled by `node-cron`. The architecture emphasizes modular React components, end-to-end TypeScript with shared Zod schemas, centralized error handling, and performance optimizations.
